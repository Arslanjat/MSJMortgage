...
const createAlert = function (params) {
  ...
  axios
    .post("https://1smtg.com/db/get_rates.php", {
      action: "create_alert",
      ...
      for (
        ...
        product.adj_port_amount.forEach(function (adj, adj_index) {
          ...
          switch (adj.adj_type.trim()) {
            case "12mo Bank Stmt":
              if (
                params.income_doc == "Bank Stmts: 12 Mo. Personal" ||
                params.income_doc == "Bank Stmts: 12 Mo. Bussiness"
              ) {
            ...
            case "24mo Bank Stmt":
              if (
                params.income_doc == "Bank Stmts: 24 Mo. Personal" ||
                params.income_doc == "Bank Stmts: 24 Mo. Bussiness"
              ) {
                new_adjustment.type = adj.adj_type;
              } else {
                return;
            ...
            case "Full Amortization":
              if (
                params.income_doc == "Full Doc" ||
                params.income_doc == "Full Doc 12(Limited)"
              ) {
                new_adjustment.type = adj.adj_type;
              } else {
                return;
            ...
            case "Loan Amount":
              if (
                (adj.adj_value == params.state || adj.adj_value == "") &&
                parseFloat(params.loan_amount) >= parseFloat(adj.amount_from) &&
                parseFloat(params.loan_amount) <= parseFloat(adj.amount_to)
              ) {
            ...
            case "standard":
              new_adjustment.type =
                "Standard " + adj["fico_from"] + "-" + adj["fico_to"];
            ...
            case "FICO":
              new_adjustment.type =
                adj.adj_type + " " + adj["fico_from"] + "-" + adj["fico_to"];
        ...
        if (parseFloat(params.fixed_term) > 180 || params.category_type == 2) {
          product.price_adjustments.forEach(function (adj, adj_index) {
            ...
            if (
              adj["purchase"] == "no" &&
              params.loan_purpose == "lp_purchase"
            ) {
              return;
            }
        ...
        product.amount_adjustments.forEach(function (adj, adj_index) {
          ...
          let adjrange =
            " Amount >= " + adj["amount_from"] + " <= " + adj["amount_to"];
        ...
        product.ms_adjustments.forEach(function (adj, adj_index) {
          ...
          if (new_adjustment.type == "STATE > 15") {
            if (
              adj["adj_value"] != params.state ||
              parseInt(params.fixed_term) <= 180
            ) {
              return;
            }
          ...
          if (
            new_adjustment.type == "Primary Residence" &&
          ...
          if (
            new_adjustment.type == "Self Employed" &&
            params.self_employed != "yes"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Purchase" &&
            new_adjustment.adj_value != ""
          ) {
            let fico_data = new_adjustment.adj_value.split("-");

            if (fico_data.length > 1) {
              if (
                params.level_fico < parseFloat(fico_data[0]) ||
                params.level_fico > parseFloat(fico_data[1])
              ) {
                return;
              }
            }
          ...
          if (
            new_adjustment.type == "Rate & Term" &&
            params.loan_purpose != "lp_rt"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "HB Rate & Term" &&
            (params.high_balance != "yes" || params.loan_purpose != "lp_rt")
          ) {
            return;
          }

          if (
            new_adjustment.type == "HB Cashout" &&
            (params.high_balance != "yes" ||
              params.loan_purpose != "lp_cashout")
          ) {
            return;
          }

          if (
            new_adjustment.type == "ARM" &&
            params.amortization_type != "arm"
          ) {
            return;
          }

          if (
            new_adjustment.type == "ARM Streamline" &&
            (params.amortization_type != "arm" || params.streamline != "yes")
          ) {
            return;
          }

          if (
            new_adjustment.type == "ARM Purchase" &&
            (params.amortization_type != "arm" ||
              params.loan_purpose != "lp_purchase")
          ) {
            return;
          ...
          if (
            new_adjustment.type == "ARM Rate & Term" &&
            (params.amortization_type != "arm" ||
              params.loan_purpose != "lp_rt")
          ) {
            return;
          ...
          if (
            new_adjustment.type == "ARM Cashout" &&
            (params.amortization_type != "arm" ||
              params.loan_purpose != "lp_cashout")
          ) {
            return;
          ...
          if (
            new_adjustment.type == "HB ARM" &&
            (params.high_balance != "yes" || params.amortization_type != "arm")
          ) {
            return;
          }

          if (
            new_adjustment.type == "HB Cashout ARM" &&
            (params.high_balance != "yes" ||
              params.amortization_type != "arm" ||
              params.loan_purpose != "lp_cashout")
          ) {
          ...
          if (
            new_adjustment.type == "HB ARM Streamline" &&
            (params.high_balance != "yes" ||
              params.amortization_type != "arm" ||
              params.streamline != "yes")
          ) {
            return;
          }
          ...
          if (new_adjustment.type == "HB 3-4 Units") {
            if (
              params.high_balance != "yes" ||
              (params.units != "3unit" && params.units != "4unit")
            ) {
              return;
            } else {
              has_3unit = true;
          ...
          if (
            new_adjustment.type == "Refinance Charge" &&
            params.loan_purpose == "lp_purchase"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Condo-Hotel" &&
            params.legal_structure != "condo"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Attached Condo Term > 15Y" &&
            params.legal_structure != "condo"
          ) {
            return;
          }
          ...
          //PENDIENTE FILTRAR EL TERM
          if (
            new_adjustment.type == "Primary Residence <= 15YR" &&
            params.occupancy != "primary"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Primary Residence <= 15YR" &&
            params.legal_structure != "single"
          ) {
            return;
          }

          if (
            new_adjustment.type == "Subordinate Financing" &&
            params.sub_financing != "yes"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Streamline" &&
            params.streamline != "yes"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "HP/HR LI" &&
            params.limited_income != "yes"
          ) {
            return;
          }

          if (
            new_adjustment.type == "Interest Only" &&
            params.interest_only != "yes"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Fixed Rate" &&
            params.amortization_type != "fixed"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Foreign National" &&
            params.foreign_national != "yes"
          ) {
            return;
          }
          ...
          if (
            new_adjustment.type == "Escrow Waiver" &&
            params.waive_ecrows != "yes"
          ) {
            return;
          }

          if (
            new_adjustment.type == "No Escrow Waiver" &&
            params.waive_ecrows == "yes"
          ) {
            return;
          }
        ...
        if (product.margins) {
          ...
          if (product.margins.relationship == "Broker") {
            ...
            product.fee =
              parseFloat(product.margins.lender_fee) +
              parseFloat(product.margins.processing_fee);
          } else {
          ...
          let comp_money =
            product.compensation * 0.01 * parseFloat(params.loan_amount);

          if (comp_money > parseFloat(product.margins.maximun)) {
            product.compensation =
              (parseFloat(product.margins.maximun) /
                parseFloat(params.loan_amount)) *
              100;
          }

          let org_money =
            product.margins.origination * 0.01 * parseFloat(params.loan_amount);

          if (org_money > parseFloat(product.margins.maximun)) {
            product.margins.origination =
              (parseFloat(product.margins.maximun) /
                parseFloat(params.loan_amount)) *
              100;
          }
          ...
          if (params.origination_charge == "yes" && params.fees_in != "yes") {
            product.origination_charge =
              parseFloat(product.margins.origination) *
              0.01 *
              parseFloat(params.loan_amount);
            product.compensation = 0;
          }
          ...
          if (params.fees_in == "yes") {
            product.fee_points =
              (product.fee / parseFloat(params.loan_amount)) * 100;
        ...
        product.base_rates.forEach(function (rates, ri) {
          ...
          let pi =
            loan_amount /
            ((Math.pow(1 + anual_rate, term) - 1) /
              (anual_rate * Math.pow(1 + anual_rate, term)));
          ...
          locks.forEach(function (l, li) {
            ...
            let lock_price =
              ...
              parseFloat(product.compensation) -
              parseFloat(product.fee_points);
        ...
        if (
          product.price_adjustments.length < 1 &&
          params.loan_purpose == "lp_purchase" &&
          (parseFloat(params.fixed_term) > 180 || params.category_type == 2)
        ) {
        ...
        if (
          params.sub_financing == "yes" &&
          product.sub_adjustments.length < 1
        ) {
