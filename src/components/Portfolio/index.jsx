...
import portfolio1Data from "../../data/portfolio.json";
...
const Portfolio = () => {
      ...
      try {
        ...
        if (data.length > 0) {
          const filteredProducts = {
            Jumbo: data.filter(
              (product) =>
                product.program.includes("JUMBO") &&
                product.program.includes("JUMBO 30Y FIXED")
            ),
            Non_QM: data.filter(
              (product) =>
                product.program.includes("NON") &&
                product.program.includes("NON QM 30Y FIXED")
            ),
          };
  ...
  const handleViewDetails = (product) => {
    try {
      ...
      console.log("Parsed Product:", parsedProduct); // Check parsed product structure
      setDetailsProduct(parsedProduct);
      setModalShow(true);
    } catch (error) {
      console.error("Error handling product details:", error);
      // Optionally, set a fallback state or notify the user about the error
    }
  };

  console.log("Details Product:", detailsProduct); // Check detailsProduct in Government component
