        ...
        {product ? (
          <div>
            ...
            <h6 style={{fontSize:"16px"}}>A. Origination Charges</h6>
              <tr>
                  <td style={{ margin: "0px", padding:"3px", paddingLeft: "20px", fontSize:"14px" }}>
      ...
      try {
        ...
        if (data.length > 0) {
          const filteredProducts = {
            Conforming: data.filter(
              (product) =>
                product.program.includes("CONFORMING 30Y") &&
                product.program.includes("CONFORMING 30Y FIXED")
            ),
            HomeReady: data.filter(
              (product) =>
                product.program.includes("HOME READY") &&
                product.program.includes("HOME READY 30Y FIXED")
            ),
            HomePossible: data.filter(
              (product) =>
                product.program.includes("HOME POSSIBLE") &&
                product.program.includes("HOME POSSIBLE 30Y FIXED")
            ),
            ConformingArm: data.filter(
              (product) =>
                product.program.includes("CONFORMING 10/6") &&
                product.program.includes("CONFORMING 10/6 ARM")
            ),
            RefiNow: data.filter(
              (product) =>
                product.program.includes("REFI NOW") &&
                product.program.includes("REFI NOW")
            ),
            RefiPossible: data.filter(
              (product) =>
                product.program.includes("REFI POSSIBLE") &&
                product.program.includes("REFI POSSIBLE")
            ),
          };
    ...
    <section id="faq" className="container-fluid mt-30">
      <div className="container col-sm-12">
        <ul id="faq-list" className="wow fadeInUp gallery twsty">
          {portfolio1Data.purchase.map((item, index) => (
